{% materialization insert_by_period,
adapter = 'fabric' -%}
...
{%- set date_source_models = config.get(
    'date_source_models',
    default = none
) -%}
{%- set unique_key = config.get(
    'unique_key',
    default = none
) -%}
{%- set start_stop_dates = get_start_stop_dates(
    timestamp_field,
    date_source_models
) | as_native -%}
{%- set period = config.get(
    'period',
    default = 'day'
) -%}
{%- set to_drop = [] -%}
...
{% if existing_relation is none %}
    {% set filtered_sql = replace_placeholder_with_period_filter(
        sql,
        timestamp_field,
...
{% else %}
    ...
    {%- set target_cols_csv = target_columns | map(
        attribute = 'quoted'
    ) | join(', ') -%}
    ...
    {% for i in range(
        ...
        {%- set period_of_load = get_period_of_load(
            period,
            i,
            period_boundaries.start_timestamp
        ) -%}
        {{ dbt_utils.log_info(
            "Running for {} {} of {} ({}) [{}]".format(
                period,
                iteration_number,
                period_boundaries.num_periods,
                period_of_load,
        ...
        {% set tmp_table_sql = get_period_filter_sql(
            target_cols_csv,
            sql,
            timestamp_field,
            period,
        ...
        {%- do loop_vars.update(
            { 'sum_rows_inserted': sum_rows_inserted }
        ) %}
        {{ dbt_utils.log_info(
            "Ran for {} {} of {} ({}); {} records inserted [{}]".format(
                period,
                iteration_number,
                period_boundaries.num_periods,
                period_of_load,
