{% macro get_start_stop_dates(
    ...
    FROM
        ...
        {% set min_max_dates ={ "start_date": start_date,
        "stop_date": stop_date } %}
...
{% macro check_placeholder(
        model_sql,
        placeholder = '__PERIOD_FILTER__'
    ) %}
    {%- if model_sql.find(placeholder) == -1 -%}
        {%- set error_message -%}
        model '{{ model.unique_id }}' does NOT include THE required STRING '__PERIOD_FILTER__' IN its SQL {%- endset -%}
        {{ exceptions.raise_compiler_error(error_message) }}
    {%- endif -%}
{% endmacro %}

{%- macro replace_placeholder_with_period_filter(
        core_sql,
        timestamp_field,
        start_timestamp,
        stop_timestamp,
    ...
    {%- set period_filter -%}
    (CAST({{ timestamp_field }} AS DATE) >= DATEADD({{ period }}, {{ offset }}, CAST('{{ start_timestamp }}' AS DATE))
    AND CAST({{ timestamp_field }} AS DATE) < DATEADD({{ period }}, {{ offset }} + 1, CAST('{{ start_timestamp }}' AS DATE)))
    AND (CAST({{ timestamp_field }} AS DATE) >= CAST('{{start_timestamp}}' AS DATE)) {%- endset -%}
    {%- set filtered_sql = core_sql | replace(
        "__PERIOD_FILTER__",
        period_filter
    ) -%}
...
{%- macro get_period_filter_sql(
    ...
    {%- do filtered_sql.update(
        { 'sql': replace_placeholder_with_period_filter(
            filtered_sql.sql,
            timestamp_field,
            start_timestamp,
            stop_timestamp,
